...
### Done / historical — gaps identified for the 300% requirement (all fixed)

> **Status note:** the four gaps below were identified when this research was compiled
> and have since **all been implemented** in `index.html`. They are kept only as a
> historical record of what the research led to. Do not re-fix these.

1. ~~Text size picker max is 1.5x (27px)~~ — **fixed**: `xl` (200%), `xxl` (250%), and `max` (300%) options exist in the picker and in `getTextSizeScale()`
2. ~~`largeTextLayout` check misses new sizes~~ — **fixed**: `applyLayoutMode()` checks `["largest", "huge", "xl", "xxl", "max"]`
3. ~~Modal header is not sticky~~ — **fixed**: `body.phone-safe-layout .modal-header` is `position: sticky` with a surface background so Close stays reachable
4. ~~`textSizeLandscapeHint` only shows for "huge"~~ — **fixed**: the hint covers `huge/xl/xxl/max`

---

## 8. Recommended Changes for `streets/index.html` (historical — implemented)

> **Status note:** the changes below were implemented in `index.html` as recommended.
