# scripts/run-gitleaks-staged.ps1 — scan the Git index snapshot for secrets.
# Canonical version for agent-tooling. Product repos reference this directly.
...
try {
  ...
  if ($indexedPaths.Count -eq 0) { exit 0 }

  Ensure-Gitleaks

  $tempRoot = Join-Path $env:TEMP "preflight-gitleaks-$([guid]::NewGuid().ToString('N'))"
